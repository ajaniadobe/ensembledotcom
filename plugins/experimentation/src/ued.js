...
function murmurHash3GC(key, seed) {
...
function getBucket(saltedId) {
  const hash = murmurHash3GC(saltedId, 0);
...
function evaluateExperiment(context, experiment) {
  ...
  const a = experiment.randomizationUnit;
  const randomizationUnit = a === 0 ? RandomizationUnit.VISITOR : a;
...
function traverseDecisionTree(decisionNodesMap, context, currentNodeId) {
  const a = decisionNodesMap[currentNodeId]; const { experiment } = a; const
    { type } = a;
